<template>
	<div class="Layout" v-if="canLoad"
		:class="{ isShowHeader: showHeader, isShowTracklist: showTracklist, isShowControls: showControls }">
		<FetchData v-if="isOnline" :channelId="channelId" :channelSlug="channelSlug" :trackId="trackId"
			:currentChannel="channel" :currentTracks="tracks" @afterFetch="updateData">
		</FetchData>

		<SpeechDispatcher v-if="speech" :isPlaying="isPlaying" :track="track"></SpeechDispatcher>

		<div class="Layout-header">
			<channel-header :isOnline="isOnline" :channel="channel" :image="image"
				:playlistPermalink="playlistPermalink" :platform="platform" :track="track"></channel-header>
		</div>
		<div class="Layout-section">
			<div class="Layout-aside">
				<provider-player :autoplay="autoplay" :isMuted="isMuted" :isPlaying="isPlaying" :track="track"
					:volume="internalVolume" @play="play" @pause="pause" @mediaNotAvailable="mediaNotAvailable"
					@trackEnded="trackEnded"></provider-player>
			</div>
			<div class="Layout-main">
				<tracklist :currentTrackIndex="currentTrackIndex" :channelSlug="channel.slug" :track="track"
					:tracks="tracksPool" :query="userQuery" :queryPermalink="queryPermalink" :platform="platform"
					@select="playTrack"></tracklist>
			</div>
		</div>
		<div class="Layout-footer">
			<player-controls :isDisabled="!this.tracksPool.length" :isMuted="isMuted" :isPlaying="isPlaying"
				:isShuffle="isShuffle" :volume="volume" @play="play" @pause="pause" @toggleMute="toggleMute"
				@toggleShuffle="toggleShuffle" @next="playNextTrack"></player-controls>
		</div>
	</div>
